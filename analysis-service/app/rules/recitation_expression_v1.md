...
拖音不是 alignment token 总时长异常，而是音节自身的有效发声部分相对局部语流出现明显、持续且可感知的延长。优先依据 `effective_voiced_duration_ratio` 和有声连续性；低能量尾部及后续静音归入 `pause_after`，不能用来生成拖音。普通句尾或意群末延长、focus 的轻微放慢都不自动显示 `——`。`duration_outlier` 只是内部候选，仅高置信度的明确 `prolongation` 进入教学图谱。拖音与句尾语调可以同时存在。拖音由声学证据和保守的系统规则生成，语言模型不重复测量。

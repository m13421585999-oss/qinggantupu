...
def robust_z(value: float | None, values: list[float | None]) -> float | None:
    pool = finite(values)
...
def semitone_normalize(value: float | None, values: list[float | None]) -> float | None:
...
def trend_label(values: list[float | None]) -> str:
    ...
    slope = float(np.polyfit(x, y, 1)[0]) if len(points) > 1 else 0.0
    if slope > 0.22:
        return "rising"
    if slope < -0.22:
        return "falling"
...
def macro_contour(indexes: list[int], values: list[float | None], zones: int = 5) -> list[dict[str, Any]]:
    if not indexes:
        return []

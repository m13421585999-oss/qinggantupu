...
“声图”把当前作品的准确正文、可选参考朗诵和场景视觉整理为可理解、可编辑的情感图谱。参考音频既可以来自优秀真人朗诵，也可以由文稿自动生成；文稿直出图谱不要求音频。

当前有三条可组合的创作路径：

1. **文稿直出图谱（本地优先）**：保存正文后创建持久化文稿分析任务，前端轮询状态；长文按 8～10 个 Sentence 分块，已完成分块会缓存并在中断后复用。任务完成后保存 `control_spec`，随后可独立生成场景图。
2. **真人参考朗诵**：上传优秀真人朗诵，由 Eleven Voice Changer 统一为标准声音，再执行 Forced Alignment、FFmpeg/Praat-Parselmouth 声学分析和结构化解释。
3. **AI 参考朗诵**：GPT-5.6 Sol 生成独立的 `performancePlan` 与 Eleven v3 `ttsText`；程序确认正文未被改写后，由固定标准 Voice 生成参考朗诵，再复用同一套音频分析链路。

正文、任务状态、控制谱版本与视觉元数据保存在 D1；音频和正式视觉资产保存在 R2。文稿分析、音频分析和视觉生成彼此独立，任一路径失败都保留真实状态与已经完成的结果，不使用固定作品或 Demo 控制谱回退。

编辑器提供两个并列版本：

- **完整版**：多场景图片、拼音、朗诵标识、语势曲线、音频同步、A4 分页与 PDF，主要页面和交互保持稳定。

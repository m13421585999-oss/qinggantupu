...
系统已经形成一套可用的“正文 → 控制谱 → 人工精修 → 完整版/紧凑版 → 多页 PDF”流程，但当前仍有三类需要优先收口的问题：

1. 当前分支有大量尚未提交的后续编辑器修改；
2. 本地 `.dev.vars` 和 `analysis-service/.env` 当前缺失，一键启动和自动分析不能直接使用；
3. 文稿分析结束后的“整篇场景图自动生成”存在前后端请求契约不一致，尚未完全闭环。
...
但 Worker 的任务入口要求 `type = scene` 时必须提供具体 `sceneId`。这会导致“图谱成功，但自动同步场景图未完成”。

建议统一为以下二选一契约：

1. `{ "type": "all", "includePlan": true }`：规划并生成整篇所有场景；
2. 增加明确的 `{ "type": "all_scenes" }`，只生成全部 Scene，不处理 Hero。

修复前应先补充端到端测试，再调整接口，避免再次影响手工单图重生成。
...
修复前端“整篇 Scene”请求和 Worker “单 sceneId”要求不一致的问题，并增加端到端测试。
...
2. 备份当前 D1/R2；
3. 提交当前未提交编辑器修改；
4. 运行 build、前端测试、Python 测试和 12 篇 PDF 回归；
5. 固化 12 篇作品清单与预期页数。

### 第二阶段：修复自动化闭环

1. 修复整篇 Scene 自动生成契约；
2. 增加文稿分析 → ControlSpec → 图片 → 编辑器的端到端测试；
3. 增加跨行语义重音硬校验；
4. 给任务增加可读的真实阶段进度，不再只显示 15/55/100。
